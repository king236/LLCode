(function($){
	...
	function getCurrCell(target){
		...
		if (cell.length){
			return {
				index: $(cell).parent().attr("node-id"),
	...
	function editFirstCell(target, index){
		var opts = $(target).datagrid('options');
		var column = opts['columns'][0];
		for(var temp in column){
			if(column[temp].editor!=null){
				editCell(target, {index:index,field:column[temp]['field']});
				break;
			}
		}
	}
	...
	function initBorder(column){
		for(var temp in column){
			if(column[temp].editor != null){
				column[temp]['styler']=function(){
					return "background-image: url('"+_path+"/images/r_tubiao.png?r=1');background-repeat:no-repeat;background-position: 100% 5%;";
				}
			}
		}
	}
	...
	function editNewRowCells(target){//依次打开最近新增行的列编辑（间接实现行的验证）
		var dg = $(target);
		var opts = dg.datagrid('options');
		var column=opts['columns'][0];
		var rows = dg.datagrid('getRows');
		$.each(rows,function(i,row){
			if(row['_validate']!=null){
				for(var temp in column){
					if(column[temp].editor != null && column[temp].editor.options != null){//可编辑列
						//仅判断为空，或自定义验证
						if(column[temp].editor.options.required || column[temp].editor.options.validType!=null){
							dg.datagrid('editCell', {index: i,field:column[temp].field});
						}
					}
				}
	...
	$.extend($.fn.datagrid.methods, {
		...
		addRow:function(jq,p){
			return jq.each(function(jq, index){
				var dg = $(this);
				var opts = dg.datagrid('options');
				var tempIndex = dg.datagrid('getRows').length;
				var cell = dg.datagrid('cell');	// current cell information
				if (cell){
					if(dg.datagrid('validateRow', cell.index)){
						endCellEdit(this, true);
					}else{
				...
				if (typeof p.index == 'undefined'){
					p['_validate'] = '';
					dg.datagrid('insertRow', {index:tempIndex,row:p});
				} else {
					p.row['_validate'] = '';
					dg.datagrid('insertRow', {index:p.index,row:p.row});
				}
				//开启第一个可编辑列
				//editFirstCell(this, tempIndex);
			});
		},
		validate:function(jq){
			editNewRowCells(jq[0]);
			var cell = $(jq[0]).datagrid('cell');	// current cell information
			if (cell){
				if($(jq[0]).datagrid('validateRow', cell.index)){
					endCellEdit(jq[0], true);
					return true;
				}else{
					return false;
				}
		...
		getSelected:function(jq){
			var cell = $(jq[0]).datagrid('cell');	// current cell information
			if($(jq[0]).datagrid('cell')==null){
				return null;
			}
			return $(jq[0]).treegrid('find', cell.index);
		},
