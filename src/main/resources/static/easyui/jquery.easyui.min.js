...
function _589(_594,_595){
	...
	var postData;
	if($(_594).prop('enctype') == 'multipart/form-data'){//文件上传提交
		postData=new FormData($(_594)[0]);
		for(var name in _595){
			postData.append(name, _595[name]);
		}
	}else{
		postData=$(_594).formToJson();
		$.extend(postData,_595);
	}
	//根据from设置的enctype属判断提交参数类型modify by:lichicheng
	var ajaxOptions = {
        url: opts.url,
        type: "post",
        xhr: function(){
			...
			if(xhr.upload){
                xhr.upload.addEventListener("progress", function(e){
                    if(e.lengthComputable){
                        var_597=e.total;
						var_598=e.loaded||e.position;
						var_599=Math.ceil(_598*100/_597);
        ...
        complete: function(res){
            try{opts.success.call(_594, res.responseText);}catch(e){}
        }
    };
	if(opts.traditional != null){
		ajaxOptions['traditional'] = opts.traditional;
	}
	if($(_594).prop('enctype') == 'multipart/form-data'){//文件上传提交
...
if(!opts.url){
return false;
}//modify by lichicheng 添加traditional:true属性
$.ajax({type:opts.method,url:opts.url,traditional:true,data:_86e,dataType:"json",success:function(data){
_86f(data);
},error:function(){
