...
(function($){
	...
	function buildGrid(target){
		...
		$(target).datagrid($.extend({}, opts, {
			...
			onLoadSuccess: function(data){
				...
				if(data.total > 0){
					$(this).edatagrid('editRow', 0);//选中第一行
					$(this).edatagrid('selectRow',0);
	...
	$.fn.edatagrid.methods = {
		...
		validateCurrRow: function(jq, index){
			
			if(typeof(index) == 'undefined'){
				var opts = $.data(jq[0], 'edatagrid').options;
				return $(jq[0]).datagrid('validateRow', opts.editIndex);
			}
...
//				}
		...
		updateRow:function(jq, index){
			return jq.each(function(){
				var dg = $(this);
				dg.datagrid('updateRow', {index:index.index,row:index.row});
				dg.datagrid('beginEdit', index.index);
				dg.datagrid('selectRow', index.index);
			});
		},
		saveRow: function(jq){
			return jq.each(function(){
